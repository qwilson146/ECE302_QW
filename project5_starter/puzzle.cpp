...
Image<Pixel> Puzzle::toPicture() const
{
  ...
  // use board of blocks (0-8)
  // Create a new image for the rearranged blocks
  Image<Pixel> rearrangedImg(img.height(), img.width());

  // Rearrange the blocks based on the board
  for (int blockIndex = 0; blockIndex < 9; ++blockIndex)
  {
    ...
    // Copy the block from the source position to the destination position
